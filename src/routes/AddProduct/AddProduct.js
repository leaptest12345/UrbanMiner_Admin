...
function AddProduct() {
  ...
  try{
    ...
    subProduct.map(async(item,index)=>{
      if(item.img)
      {
        ...
        const starCount=dataRef(database,`/ADMIN/PRODUCT/${id}/SUB_PRODUCT/${uniqueId}/${index+1}`)
        await set(starCount,{
          id: index+1,
 ...
 const createProduct=async(downloadURL)=>{
    try{
       ...
       if(!productName)
        notify('Please Fill The ProductName',2)
      else if(!productDesc)
        notify('Please Fill The ProductDescription',2)
       else 
        notify('Product Image Is Mendatory!',2)
       }
       setLoading(false)
 ...
 const theme=useTheme()
    ...
    const addSubProduct=()=>{
      if(!subProductName)
      notify('Please Fill The Product Name!',2)
      else if(!subProductDesc)
        notify('Please Fill The Product Description',2)
      else 
      {
      setSubProduct([...subProduct,{id:uniqueId,name:subProductName,description:subProductDesc,img:subImgFile}])
      setSubProductName('')
      setSubProductDesc('')
      setSubImageFile('')
      setOpen(false)
  ...
  return (
      <div>
        ...
        style={styles.btnStyle1}>Upload Product</Button>
              {
                subProduct.length!=0?
                    ...
                    <StyledTableRow align="left" key={item.id}>
                      <StyledTableCell component="th" scope="row">
                       {index+1}
                        ...
                        <StyledTableCell align="left">{item.img?<img
                  src={item.img!=''?window.URL.createObjectURL(item.img):''}
                 loading="lazy"
                 style={styles.img}
               />:<div style={styles.img} >-</div>}</StyledTableCell>
                    <StyledTableCell component="th" scope="row">
                       {item.name}
                    </StyledTableCell>
                    ...
                    <StyledTableCell align="left">
                      <Delete onClick={()=>deleteSubProduct(item)} ></Delete></StyledTableCell>
                 </StyledTableRow>
                    )}
                </TableBody>
             </Table>:null
      ...
      >
       ...
       <text>Enter Sub_product name</text><br/>
       <input
             ...
             <div>
             <h4 style={{marginTop:'5%'}}>Upload Sub_Product Image (Optional)</h4>
          <input style={{marginTop:'20px'}} type="file" onChange={(e)=>setSubImageFile(e.target.files[0])} />
             </div>
         <div>
         {subImgFile!=''?  <span>
                <img src={window.URL.createObjectURL(subImgFile)}
