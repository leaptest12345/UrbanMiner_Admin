...
export default function CustomerDetail(props)
{
   ...
   function onClick(slug,data, parameters = {}) {
    ...
    const viewPdf=(id)=>{
        ...
        try{
            const refDetail=ref(database,`/PDF/user:${userId}/customer:${customerId}/invoiceid:${id}/`)
            onValue(refDetail,snapShot=>{
                const data=formateData(snapShot.val())
    ...
    return(
        <div>
            ...
            <span style={{
                fontWeight:'bold'
            }}>UserFirstName : {user?.UserFirstName}</span><br/>
             <span style={{
                fontWeight:'bold'
            }}>UserLastName : {user?.UserLastName}</span><br/>
             <span style={{
                fontWeight:'bold'
            }}>BusinessEmail : {user?.BusinessEmail}</span><br/>
             <span style={{
                fontWeight:'bold'
            }}>BusinessName : {user?.BusinessName}</span><br/>
             <span style={{
                fontWeight:'bold'
            }}>Address : {user?.BusinessAddress}</span><br/>
     <div style={{
        display:'flex',
     ...
     {
        customerImages&&customerImages.map(item=>{
            return(
             <div style={{
                margin:20
             }}>
